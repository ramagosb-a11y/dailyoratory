...
- Content change / code change / both: Both — remove page modules, write original inclusive bridge copy, update metadata/search copy, add one presentational heavenly-themed Scripture module, and (R7) refine existing book-card controls without changing orientation content.
- In scope: Hero CTA becomes **Mass Readings Reflections**; remove the specified Catholic-only, local-state, copy, plan, FAQ, resource, source/copyright, and ancillary sections from `/bible`; replace the Catholic practice with a gentle general reading suggestion; expand Lectio Divina and the Bible-books overview; add a non-interactive heavenly visual module using existing Mass visual language. R7 replaces browser-style external-link glyphs on book cards with intentional text buttons and upgrades the existing native “Before you begin” disclosure visual treatment.
