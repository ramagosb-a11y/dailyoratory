...
## Behavior
- Page order: Step 1 reading guidance/prayer/USCCB link; Step 2 daily Scripture journal editor; Step 3 Read · Reflect · Pray hero; current Mass reading reflection; history jump button; Explore Scripture Further; supplemental resources; Journal History; month calendar; upcoming/archive section. The editor appears once. History shows all entries newest-first with copy controls.
...
- Further study: derive external links from public reading references; Douay-Rheims opens the corresponding DRBO chapter where supported. Haydock opens its corresponding commentary chapter where supported; if a precise source page cannot be resolved, link to the matching testament/book index with a clear label. Also show fixed New Advent Genesis 1 and HeavenBound links. New tabs use `rel="noopener noreferrer"`. The HeavenBound link is a plain resource link and never includes journal text.
...
| AC-10 | Daily editor and reflection steps appear in the approved order; history stays above month calendar | Render order and anchor navigation | Step 1, Step 2 editor, Step 3 reflection order; history button is immediately after reflection; resources remain before history; history precedes calendar; anchor is keyboard navigable; all entries shown; one entry per local day is stated |
