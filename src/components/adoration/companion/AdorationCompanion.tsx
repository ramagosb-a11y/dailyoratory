...
function SilenceView({
  ...
  const movements = [
    ["Adoration", "Begin with God’s holiness and loving presence."],
    ["Thanksgiving", "Name graces received before naming what you need."],
    ["Repentance", "Ask for mercy and the courage to begin again."],
    ["Intercession", "Place people and needs quietly before Jesus."],
    ["Rest", "Let words become fewer and remain in attentive love."],
  ];

  return (
    <div>
      <section className={styles.hero}><p className={styles.eyebrow}>Silent Prayer</p><h2>Remain Here With Jesus</h2><p>Choose a duration, begin with a gentle chime, and let the timer keep watch without demanding your attention.</p></section>
      <section className={styles.silencePanel}>
        <p className={styles.eyebrow}>Prayer Timer</p>
        <time dateTime={`PT${timerSeconds}S`}>{formatTime(timerSeconds)}</time>
        <div className={styles.presetRow}>{[5, 15, 30, 45, 60].map((minutes) => <button key={minutes} type="button" onClick={() => onTimerChange(minutes)}>{minutes} min</button>)}</div>
        <div className={styles.timerActions}><button type="button" className={styles.goldButton} onClick={onToggle}>{timerRunning ? "Pause Silence" : "Begin Silent Prayer"}</button><button type="button" onClick={onReset}>Reset</button><button type="button" onClick={onChime}>Test Chime</button></div>
      </section>
      <section className={styles.holyHourGrid}>
        {movements.map(([title, description], index) => <article key={title}><span>{index + 1}</span><h3>{title}</h3><p>{description}</p></article>)}
      </section>
      <p className={styles.quietNote}>A completed timer sounds only a short tone generated by your browser. No copyrighted audio or recording is used.</p>
    </div>
  );
}
