import { TrackedLink } from "@/components/analytics/TrackedLink";
import { getRelatedBibleTools } from "@/lib/bible";
import { BibleCardGrid, BibleSection } from "@/components/bible/BibleUi";

export function RelatedBibleTools() {
  const tools = getRelatedBibleTools();

  return (
    <BibleSection
      eyebrow="Related Daily Oratory tools"
      title="Keep Scripture Connected to the Rest of Catholic Life"
      summary="Daily Oratory is meant to keep Bible reading tied to prayer, worship, doctrine, family life, and daily conversion."
    >
      <BibleCardGrid columns="md:grid-cols-2 xl:grid-cols-4">
        {tools.map((tool) => (
          <TrackedLink
            key={tool.href}
            href={tool.href}
            className="card-parchment block p-5 transition hover:border-gold"
            eventName="bible_related_tool_click"
            eventParams={{
              category: "related-tools",
              item_slug: tool.title.toLowerCase().replaceAll(" ", "-"),
              source: "/bible",
              destination: tool.href,
            }}
          >
            <h3 className="font-display text-2xl font-semibold text-navy">{tool.title}</h3>
            <p className="mt-3 text-sm leading-7 text-muted">{tool.description}</p>
            <p className="mt-4 text-sm font-semibold text-navy">{tool.cta}</p>
          </TrackedLink>
        ))}
      </BibleCardGrid>
    </BibleSection>
  );
}
