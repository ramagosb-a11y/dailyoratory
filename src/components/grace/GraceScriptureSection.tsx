import { LinkedCitationText } from "@/components/LinkedCitationText";
import { TrackedLink } from "@/components/analytics/TrackedLink";
import type { GraceScriptureReference } from "@/types/grace";

export function GraceScriptureSection({ references }: { references: GraceScriptureReference[] }) {
  return (
    <section className="card-parchment p-6 sm:p-8">
      <h2 className="font-display text-4xl font-semibold text-navy">Scripture for Grace</h2>
      <div className="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {references
          .slice()
          .sort((a, b) => a.sortOrder - b.sortOrder)
          .map((reference) => (
            <article key={reference.id} className="dashboard-card p-5">
              <h3 className="font-display text-2xl font-semibold text-navy">
                <LinkedCitationText text={reference.reference} />
              </h3>
              <p className="daily-card-readable mt-4 text-base leading-7 text-muted">
                <LinkedCitationText text={reference.description} />
              </p>
              <div className="mt-5">
                <TrackedLink
                  href={reference.usccbUrl}
                  external
                  eventName="grace_related_link_click"
                  eventParams={{ section: "scripture", destination: reference.usccbUrl }}
                  className="btn btn-secondary focus-ring daily-button-readable min-h-12 justify-center"
                >
                  Open at USCCB
                </TrackedLink>
              </div>
            </article>
          ))}
      </div>
    </section>
  );
}
