    ...
    <main className="sacred-hours-main">
      ...
      <article className="sacred-moment-grid">
        ...
        <div className="sacred-content"><p className="eyebrow">{selected.liturgicalDay}{selected.displayTime ? ` · ${selected.displayTime}` : ""}</p><h2 className="font-display">{selected.title}</h2><p className="sacred-theme">{selected.subtitle}</p>
          ...
          <div className="sacred-nav"><button onClick={complete} className="btn btn-primary focus-ring">Complete &amp; Continue</button><button disabled={!next} onClick={() => next && openMoment(next)} className="btn btn-outline focus-ring">Next — Skip Completion</button><button disabled={!previous} onClick={() => previous && openMoment(previous)} className="btn btn-outline focus-ring">← Previous</button></div>
        </div>
      </article>
