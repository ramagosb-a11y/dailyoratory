...
export function SacredHoursExperience() {
  ...
  function complete() {
    ...
    setStore(next); saveHolyWeekStore(next);
    const following = holyWeekMeditations[selected.sequence] ?? selected;
    if (following !== selected) openMoment(following);
  }
  function reset() { resetHolyWeekStore(); setStore({ completedMomentIds: [], lastOpenedMoment: null, preferredMode: "full" }); setSelectedId("palm-sunday"); }
  ...
  return <div className={mood}>
    <section className="sacred-hours-hero">
      <div className="sacred-hours-hero-inner">
        ...
        <div className="sacred-hours-crossmark" aria-hidden="true">✦</div>
    ...
    <main className="sacred-hours-main">
      ...
      <div ref={momentStartRef} className="sacred-hours-toolbar"><div><p>{mode === "live" ? "Walk With Jesus Now" : `Sacred Moment ${selected.sequence} of 24`}</p><div className="sacred-progress" aria-label={`${selected.sequence} of 24 moments`}><span style={{ width: `${(selected.sequence / holyWeekMeditations.length) * 100}%` }} /></div></div><div className="sacred-hours-toolbar-actions"><button className="btn btn-primary btn-small focus-ring" onClick={complete}>Complete</button><button disabled={!next} onClick={() => next && openMoment(next)} className="btn btn-outline btn-small focus-ring">Next →</button><button className="btn btn-outline focus-ring" onClick={() => setPickerOpen(true)}>Choose a Sacred Moment</button></div></div>
      <article className="sacred-moment-grid">
        <div className="sacred-artwork" role="img" aria-label={selected.imageAlt}>{selected.imageSrc ? <Image src={selected.imageSrc} alt={selected.imageAlt} fill sizes="(max-width: 760px) 100vw, 50vw" quality={100} unoptimized priority={selected.sequence <= 2} style={{ objectFit: "cover", objectPosition: selected.imageFocalPoint ?? "center" }} /> : null}<div className="sacred-artwork-overlay" /><div className="sacred-artwork-symbol">✦</div><p>{selected.imageDescription}</p></div>
        <div className="sacred-content"><p className="eyebrow">{selected.liturgicalDay}{selected.displayTime ? ` · ${selected.displayTime}` : ""}</p><h2 className="font-display">{selected.title}</h2><p className="sacred-theme">{selected.subtitle}</p>
          <div className="scripture-card"><p className="eyebrow">Sacred Scripture · Douay-Rheims</p><strong>{selected.scriptureReference}</strong><p>“{selected.shortScriptureExcerpt}”</p>{selected.timeCertainty !== "untimed" ? <small>{certaintyLabels[selected.timeCertainty]} ⓘ</small> : null}</div>
          <section className="meditation-copy"><h3>Be still</h3>{selected.meditation.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</section>
