...
export function SacredHoursExperience() {
  ...
  return <div className={mood}>
    ...
    <main className="sacred-hours-main">
      ...
      <div ref={momentStartRef} className="sacred-hours-toolbar"><div><p>{mode === "live" ? "Walk With Jesus Now" : `Sacred Moment ${selected.sequence} of 24`}</p><div className="sacred-progress" aria-label={`${selected.sequence} of 24 moments`}><span style={{ width: `${(selected.sequence / holyWeekMeditations.length) * 100}%` }} /></div></div></div>
      <article className="sacred-moment-grid">
        <div className="sacred-artwork-frame"><div ref={artworkRef} className={`sacred-artwork${isFullscreen ? " sacred-artwork-fullscreen" : ""}`}>{selected.imageSrc ? <Image src={selected.imageSrc} alt={selected.imageAlt} fill sizes="(max-width: 1023px) 100vw, 35vw" quality={100} unoptimized priority={selected.sequence <= 2} style={{ objectFit: "cover", objectPosition: selected.imageFocalPoint ?? "center" }} /> : null}<div className="sacred-artwork-overlay" /><p>{selected.imageDescription}</p><button type="button" className="sacred-artwork-fullscreen-button btn btn-secondary" onClick={() => void toggleFullscreen()}>{isFullscreen ? "Exit fullscreen" : "View image fullscreen"}</button></div><div className="sacred-artwork-mobile-action"><button type="button" className="btn btn-secondary focus-ring" onClick={() => void toggleFullscreen()}>View full image</button></div></div>
