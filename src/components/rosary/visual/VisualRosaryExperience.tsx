    ...
    <div className={styles.experience}>
      <div className={styles.shell}>
        ...
        <section id="prayer-player" className={`${styles.panel} ${styles.prayerPlayer}`}>
          ...
          <div className={styles.playerControls}><button type="button" aria-label="Previous prayer" onClick={() => goToStep(stepIndex - 1)} disabled={stepIndex === 0}>|◀</button><button type="button" className={styles.primaryButton} onClick={() => setRunning((value) => !value)}>{running ? "Ⅱ Pause" : "▶ Start Contemplation"}</button><button type="button" aria-label="Next prayer" onClick={() => goToStep(stepIndex + 1)} disabled={stepIndex === sequence.length - 1}>▶| <span>Next Prayer</span></button><button type="button" aria-label="Restart timer" onClick={() => { setSeconds(pace); setRunning(false); }}>↻</button></div>
        </section>
