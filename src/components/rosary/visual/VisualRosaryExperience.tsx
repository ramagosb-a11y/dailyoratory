    ...
    <div className={styles.experience}>
      <div className={styles.shell}>
        ...
        <section id="bead-strand" className={`${styles.panel} ${styles.strand}`}>
          ...
          {sections.map((section) => { const indexed = sequence.map((item, index) => ({ item, index })).filter(({ item }) => item.section === section); const active = indexed.some(({ index }) => index === stepIndex); return <details key={section} open={active} className={active ? styles.activeStrand : ""}><summary>{section}<span>{active ? step.title : `${indexed.length} prayers`}</span></summary><div>{indexed.map(({ item, index }) => <button ref={index === stepIndex ? activeBeadRef : undefined} key={item.id} type="button" onClick={() => goToStep(index)} className={index === stepIndex ? styles.currentBead : index < stepIndex ? styles.completeBead : ""} aria-label={`Prayer ${index + 1}: ${item.title}`}>{item.id === "intro-creed" ? <span className={styles.beadCross} aria-hidden="true" /> : item.beadLabel}</button>)}</div></details>; })}
        </section>
      </div>
